...
import pygame
from config import MIN_LIFETIME, MAX_LIFETIME
from models import Square, State

PANIC_MULT = 4.0
CALM_MULT = 6.0


def create_random_square(width: int, height: int) -> Square:
...
def create_squares(num_squares: int, width: int, height: int) -> list[Square]:
    return [create_random_square(width, height) for _ in range(num_squares)]
...
def handle_rebirth(squares: list[Square], width: int, height: int) -> None:
    for idx, square in enumerate(squares):
        if square.lifetime >= square.max_lifetime:
            squares[idx] = create_random_square(width, height)
